...
class BaseInfluencer(ABC):
    ...
    def display_campaigns_participated(self):
        ...
        campaign_info = []
        type_of_influencer = self.__class__.__name__
        for campaign in self.campaigns_participated:
            campaign_info.append(f"  - Campaign ID: {campaign.campaign_id}, Brand: {campaign.brand}, Reached followers: {self.reached_followers(campaign.__class__.__name__)}")

        return (f"{type_of_influencer} :) {self.username} :) participated in the following campaigns:\n"
                + "\n".join(campaign_info))
